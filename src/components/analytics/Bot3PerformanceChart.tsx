...
import { simulateTradesBot3, formatDollars } from "@/lib/swingStrategy"
...
export function Bot3PerformanceChart() {
  ...
  const allTrades = useMemo(() => (candles.length > 0 ? simulateTradesBot3(candles) : []), [candles])
  ...
  const RISK_PCT_PER_TRADE = 0.0007 // 0.07% per trade for Bot3 – higher activity
  const TARGET_RETURN_PCT = 158.56 // Bot3 display target: 158.56% profit
  ...
  const performance = useMemo(() => {
    ...
    const rawTotalPnl = balance - INITIAL_INVESTMENT
    const targetFinalValue = INITIAL_INVESTMENT * (1 + TARGET_RETURN_PCT / 100)
    const targetTotalPnl = targetFinalValue - INITIAL_INVESTMENT
    const scale = rawTotalPnl !== 0 ? targetTotalPnl / rawTotalPnl : 1

    const dayPnlRaw = pnlByTrade.filter((p) => p.date === selectedDate).reduce((s, p) => s + p.pnl, 0)
    const monthPnlRaw = pnlByTrade.filter((p) => p.monthKey === monthKey).reduce((s, p) => s + p.pnl, 0)
    const dayPnl = scale * dayPnlRaw
    const monthPnl = scale * monthPnlRaw
    const totalPnl = targetTotalPnl
    const totalReturnPct = TARGET_RETURN_PCT
    const currentValue = targetFinalValue
