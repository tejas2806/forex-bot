...
interface Globe3DDemoSecondProps {
  className?: string
}

export default function Globe3DDemoSecond({ className }: Globe3DDemoSecondProps) {
  return (
    <div
      className={cn(
        "relative h-full min-h-[560px] w-full max-w-2xl overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/75 p-8 shadow-[0_18px_42px_rgba(2,6,23,0.32)] md:p-10",
        className
      )}
    >
      <h2 className="mb-3 text-2xl font-bold text-zinc-100 md:text-3xl">
        Global market network
      </h2>
      <p className="max-w-xl text-balance text-sm text-zinc-400 md:text-base">
        AlphaForge strategies are built for a worldwide trading audience. Track signals,
        bot activity, and execution confidence across major market hubs.
      </p>
      <Globe3D
        className="absolute -bottom-[23rem] -left-10 h-[820px] md:-bottom-[24rem] md:-left-4 md:h-[900px]"
